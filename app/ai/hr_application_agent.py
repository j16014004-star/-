"""LangGraph agent that creates a factual application draft."""
...
from app.ai.hr_application_prompt import SYSTEM_PROMPT, build_user_prompt
...
from app.schemas.hr import HrApplicationDraftAIOutput


class HrApplicationAgentState(TypedDict, total=False):
    job: dict[str, Any]
    resume_text: str
    ...
    result: HrApplicationDraftAIOutput


class HrApplicationAgent:
    def __init__(
        self,
        ...
        graph = StateGraph(HrApplicationAgentState)
    ...
    async def run(self, *, job: dict, resume_text: str) -> HrApplicationAgentState:
        return await self.graph.ainvoke({"job": job, "resume_text": resume_text})

    async def _retrieve(self, state: HrApplicationAgentState) -> dict[str, Any]:
        job = state.get("job") or {}
        query = " ".join(
            filter(None, [str(job.get("title") or ""), state.get("resume_text", "")[:2000]])
        )
    ...
    async def _generate(self, state: HrApplicationAgentState) -> dict[str, Any]:
    ...
    @staticmethod
    async def _validate(state: HrApplicationAgentState) -> dict[str, Any]:
        result = HrApplicationDraftAIOutput.model_validate(
